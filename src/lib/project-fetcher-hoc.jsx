...
const fetchProjectToken = projectId => {
    ...
    return fetch(`https://mainapi-scratchturbo.replit.app/api/projects/getPublished?id=${projectId}`)
        .then(r => {
            if (!r.ok) return null;
            return r.json();
        })
        .then(dataOrNull => {
            ...
            var LastFetchedProject = dataOrNull ? dataOrNull : null;
            return token;
        })
        .catch(err => {
            log.error(err);
            ...
            if (projectId === '69420') {
                ...
                if (projectUrl) {
                    ...
                    if (projectId === '0') {
                        ...
                        projectUrl = `https://mainapi-scratchturbo.replit.app/api/projects/getPublished?type=file&id=${projectId}`
                        assetPromise = progressMonitor.fetchWithProgress(projectUrl)
                            .then(r => {
                                this.props.vm.runtime.renderer.setPrivateSkinAccess(false);
                                if (!r.ok) {
