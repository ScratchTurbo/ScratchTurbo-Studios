...
const isUploadAvailable = async () => {
    let res = null;
    try {
        res = await fetch('https://mainapi-scratchturbo.replit.app/api');
    } catch {
...
class ShareButton extends React.Component {
    ...
    async handleMessageEvent(e) {
        if (!e.origin.startsWith(`https://scratchturbo.replit.app`)) {
            return;
        }
    ...
    async onUploadProject() {
        ...
        isUploadAvailable().then(available => {
            ...
            window.open(`https://scratchturbo.replit.app/${targetPage}?name=${this.props.projectTitle}${editPiece}${remixPiece}&external=${url}`, '_blank');
        });
    }
    render() {
